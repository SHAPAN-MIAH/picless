export class PeerStats {
  ...
  get currentFPS() {
    return (
      ((this.totalFramesEncodedCount - this.lastFramesEncoded) / (this.currentTimestamp - this.lastTime)) *
      1000
    ).toFixed(1)
  }
  ...
  get averageIncomingBitrate() {
    return Math.floor(
      (8 * (this.totalBytesReceivedCount - this.firstBytesReceivedCount)) / (this.currentTimestamp - this.startTime)
    )
  }
  ...
  get currentIncomingBitrate() {
    return Math.floor(
      (8 * (this.totalBytesReceivedCount - this.lastBytesReceived)) / (this.currentTimestamp - this.lastTime)
    )
  }
