...
export function tryToPlay(name, token, type, subscriberId, subscriberCode, noStreamCallback, initializePlayer) {
...
export function tryToVODPlay(name, token, subscriberId, subscriberCode, noStreamCallback, playType, initializePlayer) {
  if (typeof playType === 'undefined' || playType == null || playType.length == 0) {
    console.error('playType is not defined')
    return
  }
  const firstPlayType = playType[0]
  let secondPlayType = null
